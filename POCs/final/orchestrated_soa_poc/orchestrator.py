    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            auth_response = await client.post(
                f"{AUTH_SERVICE_URL}/authenticate", json={"user_id": patient_id}
            )
        ...
        async with httpx.AsyncClient() as client:
            symptom_response = await client.post(
                f"{SYMPTOM_SERVICE_URL}/check_symptom",
        ...
        async with httpx.AsyncClient() as client:
            appointment_response = await client.post(
                f"{APPOINTMENT_SERVICE_URL}/schedule",
                json={"patient_id": patient_id, "time": appointment_time},
                headers={"Authorization": f"Bearer {token}"},
            )
        ...
        async with httpx.AsyncClient() as client:
            notification_response = await client.post(
                ...
                json={
                    ...
                    "message": f"Your appointment is scheduled. ID: {appointment_id}",
                },
                headers={"Authorization": f"Bearer {token}"},
            )
