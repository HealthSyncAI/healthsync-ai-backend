...
async def setup_data_for_statistics():
    ...
    async with TestAsyncSessionLocal() as session:
        ...
        p1 = User(
            username="stat_p1",
            email="stat_p1@example.com",
            hashed_password="p",
            role=UserRole.patient,
        )
        p2 = User(
            username="stat_p2",
            email="stat_p2@example.com",
            hashed_password="p",
            role=UserRole.patient,
        )
        d1 = User(
            username="stat_d1",
            email="stat_d1@example.com",
            hashed_password="d",
...
async def test_get_usage_statistics(setup_data_for_statistics):
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        try:
            ...
            print(
                f"GET Statistics Response Data: {json.dumps(response_data, indent=2)}"
            )
        ...
        assert (
            set(response_data.keys()) == expected_keys
        ), "Response keys do not match UsageStatistics schema"

        assert (
            response_data["total_users"]
            == expected_counts["patient_count"] + expected_counts["doctor_count"]
        )
