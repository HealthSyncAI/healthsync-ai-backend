...
async def dummy_get_current_user_method(
    user_id=1, role="patient", username="dummy_patient"
):
...
async def test_appointment_endpoints(setup_users_for_appointment, monkeypatch):
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        appointment_payload = {
            ...
            "end_time": datetime(
                2025, 4, 15, 14, 30, 0, tzinfo=timezone.utc
            ).isoformat(),
            "telemedicine_url": "https://meet.example/appt123",
        }
        response_post = await client.post(
            "/api/appointment/", json=appointment_payload, headers=headers
        )
        ...
        ), f"Expected 201, got {response_post.status_code}. Response: {response_post.text}"
        ...
        response_get_hr = await client.get(
            f"/api/appointment/{appointment_id}/health-records", headers=headers
        )
        ...
        assert (
            response_get_hr.status_code == 200
        ), f"Expected 200, got {response_get_hr.status_code}. Response: {response_get_hr.text}"
        assert isinstance(response_data_hr, list)

        assert (
            len(response_data_hr) == 0
        ), "Expected empty health records list due to mocking"
        print("Health records fetched successfully (expected empty list).")

        print("\n--- Testing GET /api/appointment/doctors ---")

        _dummy_auth_service_instance.set_user(patient_id, "patient", "appt_patient")
        response_get_doctors = await client.get(
        ...
        try:
            ...
            print(
                f"GET Doctors Response Data: {json.dumps(response_data_doctors, indent=2)}"
            )
        except json.JSONDecodeError:
        ...
        assert (
            response_get_doctors.status_code == 200
        ), f"Expected 200, got {response_get_doctors.status_code}. Response: {response_get_doctors.text}"
        assert isinstance(response_data_doctors, list)
        assert len(response_data_doctors) > 0, "Expected at least one doctor"
        found_doctor = next(
            (d for d in response_data_doctors if d["id"] == doctor_id), None
        )
        ...
        response_get_doctor_detail = await client.get(
            f"/api/appointment/doctors/{doctor_id}", headers=headers
        )
        print(
            f"GET Doctor Detail Response Status: {response_get_doctor_detail.status_code}"
        )
        try:
            response_data_doctor_detail = response_get_doctor_detail.json()
            print(
                f"GET Doctor Detail Response Data: {json.dumps(response_data_doctor_detail, indent=2)}"
            )
        except json.JSONDecodeError:
        ...
        assert (
            response_get_doctor_detail.status_code == 200
        ), f"Expected 200, got {response_get_doctor_detail.status_code}. Response: {response_get_doctor_detail.text}"
