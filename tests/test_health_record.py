...
async def dummy_get_current_user_method(
    user_id=1, role: UserRole = UserRole.patient, username="dummy_patient"
):
...
async def test_health_record_endpoints(setup_users_for_health_record):
    ...
    async with AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        response_post_generic = await client.post(
            "/api/health-record/", json=generic_record_payload, headers=headers
        )
        print(f"POST Generic Response Status: {response_post_generic.status_code}")
        try:
            response_data_generic = response_post_generic.json()
            print(
                f"POST Generic Response Data: {json.dumps(response_data_generic, indent=2)}"
            )
        ...
        assert (
            response_post_generic.status_code == 201
        ), f"Expected 201, got {response_post_generic.status_code}. Response: {response_post_generic.text}"
        ...
        print(
            f"Generic health record created successfully. ID: {record_ids['generic']}"
        )
        ...
        response_post_forbidden = await client.post(
            "/api/health-record/", json=forbidden_payload, headers=headers
        )
        print(f"POST Forbidden Response Status: {response_post_forbidden.status_code}")

        assert (
            response_post_forbidden.status_code == 201
        ), f"Expected 201 (patient creating for self), got {response_post_forbidden.status_code}. Detail: {response_post_forbidden.text}"
        ...
        doctor_note_payload = {
            ...
            "patient_id": patient_id,
            "symptoms": [
                {"name": "headache", "severity": 3, "description": "tension-type"}
            ],
            "diagnosis": [{"name": "Tension headache", "icd10_code": "G44.2"}],
            "treatment_plan": [
                {
                    "description": "Continue rest, follow up if worsens",
                    "follow_up": "PRN",
                }
            ],
            "medication": [
        ...
        response_post_note = await client.post(
            "/api/health-record/doctor-note", json=doctor_note_payload, headers=headers
        )
        print(f"POST Note Response Status: {response_post_note.status_code}")
        try:
            response_data_note = response_post_note.json()
            print(
                f"POST Note Response Data: {json.dumps(response_data_note, indent=2)}"
            )
        except json.JSONDecodeError:
        ...
        assert (
            response_post_note.status_code == 201
        ), f"Expected 201, got {response_post_note.status_code}. Response: {response_post_note.text}"
        ...
        _dummy_auth_service_instance.set_user(patient_id, "patient", "hr_patient")
        response_get_patient_recs = await client.get(
            f"/api/health-record/patient/{patient_id}", headers=headers
        )
        print(
            f"GET Patient Recs Response Status: {response_get_patient_recs.status_code}"
        )
        try:
            response_data_patient_recs = response_get_patient_recs.json()
            print(
                f"GET Patient Recs Response Data: {json.dumps(response_data_patient_recs, indent=2)}"
            )
        except json.JSONDecodeError:
        ...
        assert (
            response_get_patient_recs.status_code == 200
        ), f"Expected 200, got {response_get_patient_recs.status_code}. Response: {response_get_patient_recs.text}"
        assert isinstance(response_data_patient_recs, list)

        assert (
            len(response_data_patient_recs) == 3
        ), f"Expected 3 records, found {len(response_data_patient_recs)}"
        found_ids = {rec["id"] for rec in response_data_patient_recs}
        assert (
            record_ids["generic"] in found_ids
        ), "Generic record (by doctor) not found"
        assert record_ids["note"] in found_ids, "Doctor note record not found"
        assert (
            record_ids["patient_self_generic"] in found_ids
        ), "Generic record (by patient) not found"
        ...
        _dummy_auth_service_instance.set_user(patient_id, "patient", "hr_patient")
        response_get_specific = await client.get(
            f"/api/health-record/{note_id_to_fetch}", headers=headers
        )
        ...
        try:
            ...
            print(
                f"GET Specific Rec Response Data: {json.dumps(response_data_specific, indent=2)}"
            )
        except json.JSONDecodeError:
        ...
        assert (
            response_get_specific.status_code == 200
        ), f"Expected 200, got {response_get_specific.status_code}. Response: {response_get_specific.text}"
